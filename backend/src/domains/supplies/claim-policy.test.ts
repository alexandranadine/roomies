...
void describe('decideSupplyClaim', () => {
  void it('names the supply.claim action', () => {
    assert.equal(SUPPLY_ACTION.claim, 'supply.claim');
  });

  void it('allows an active Roommate through ordinary capability', () => {
    assert.deepEqual(
      decideSupplyClaim({ actor: actor('ROOMMATE'), targetHomeId: HOME_A }),
      { allowed: true },
    );
  });
  ...
  void it('allows an active Admin through the same ordinary capability', () => {
    assert.deepEqual(
      decideSupplyClaim({ actor: actor('ADMIN'), targetHomeId: HOME_A }),
      { allowed: true },
    );
    assert.deepEqual([...SUPPLY_CLAIM_CAPABLE_ROLES], ['ROOMMATE', 'ADMIN']);
  });

  void it('denies a Home-scope mismatch for both roles', () => {
    assert.deepEqual(
      decideSupplyClaim({ actor: actor('ROOMMATE'), targetHomeId: HOME_B }),
      { allowed: false, reason: 'HOME_SCOPE_MISMATCH' },
    );
    assert.deepEqual(
      decideSupplyClaim({ actor: actor('ADMIN'), targetHomeId: HOME_B }),
      { allowed: false, reason: 'HOME_SCOPE_MISMATCH' },
    );
  });

  void it('does not grant supply.claim to an inactive Membership', () => {
    assert.equal(
      (SUPPLY_CLAIM_CAPABLE_ROLES as readonly string[]).includes('INACTIVE'),
      false,
    );
  });
  ...
  void it('has no Admin-bypass special case', async () => {
    ...
    assert.match(source, /SUPPLY_CLAIM_CAPABLE_ROLES/);
  });
});
