...
void describe('decideSupplyCancel', () => {
  void it('names the supply.cancel action', () => {
    assert.equal(SUPPLY_ACTION.cancel, 'supply.cancel');
  });

  void it('allows an active Roommate through ordinary capability', () => {
    assert.deepEqual(
      decideSupplyCancel({ actor: actor('ROOMMATE'), targetHomeId: HOME_A }),
      { allowed: true },
    );
  });
  ...
  void it('allows an active Admin through the same ordinary capability', () => {
    assert.deepEqual(
      decideSupplyCancel({ actor: actor('ADMIN'), targetHomeId: HOME_A }),
      { allowed: true },
    );
    assert.deepEqual([...SUPPLY_CANCEL_CAPABLE_ROLES], ['ROOMMATE', 'ADMIN']);
  });

  void it('denies a Home-scope mismatch for both roles', () => {
    assert.deepEqual(
      decideSupplyCancel({ actor: actor('ROOMMATE'), targetHomeId: HOME_B }),
      { allowed: false, reason: 'HOME_SCOPE_MISMATCH' },
    );
    assert.deepEqual(
      decideSupplyCancel({ actor: actor('ADMIN'), targetHomeId: HOME_B }),
      { allowed: false, reason: 'HOME_SCOPE_MISMATCH' },
    );
  });

  void it('does not grant supply.cancel to an inactive Membership', () => {
    assert.equal(
      (SUPPLY_CANCEL_CAPABLE_ROLES as readonly string[]).includes('INACTIVE'),
      false,
    );
  });
  ...
  void it('has no Admin-bypass, creator, or claimant special case', async () => {
    ...
    assert.match(source, /SUPPLY_CANCEL_CAPABLE_ROLES/);
  });
});
