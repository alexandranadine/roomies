  ...
  void it(
    ...
    async () => {
      ...
      try {
        ...
        const homeIds = [homeAId, homeBId, archivedHomeId, inaccessibleHomeId];
        await database.pool.query(
          `UPDATE memberships
           SET ended_by_membership_id = id
           WHERE home_id = ANY($1) AND ended_at IS NOT NULL`,
          [homeIds],
        );
        await database.pool.query(
          `DELETE FROM memberships
           WHERE home_id = ANY($1) AND ended_at IS NULL`,
          [homeIds],
        );
        const remainingMemberships = await database.pool.query<{ id: string }>(
          'SELECT id FROM memberships WHERE home_id = ANY($1)',
          [homeIds],
        );
