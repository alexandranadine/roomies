...
void describe('Task HTTP PostgreSQL', () => {
  void it(
  ...
  void it(
    'covers create/list authorization, Origin, ordering, and safe DTOs',
      ...
      try {
        ...
        await withAppServer(app, async (request) => {
          async function signUp(name: string) {
            const email = `m31a-${name}-${randomUUID()}@example.test`;
          ...
          assert.equal(createdBody.title, 'Dated chore');
          assert.equal(createdBody.status, 'OPEN');
          assert.equal(createdBody.source, 'MANUAL');
          assert.equal(createdBody.scheduledFor, '2026-09-15');
          assert.equal('taskDefinitionId' in (created.json() as object), false);
          assert.equal('completedAt' in (created.json() as object), false);
          ...
          const later = await request({
            ...
            body: JSON.stringify({ title: 'Undated chore' }),
          });
          ...
          const listBody = listed.json() as TaskDto[];
          assert.equal(listBody.length, 2);
          assert.equal(listBody[0]?.title, 'Dated chore');
          assert.equal(listBody[1]?.title, 'Undated chore');
          assert.deepEqual(Object.keys(listBody[0] ?? {}), [
            'id',
            'title',
            'status',
            'source',
          ...
          const archivedList = await request({
            method: 'GET',
            path: `/api/v1/homes/${archivedHome}/tasks`,
            headers: { Cookie: roommate.cookie },
          });
          assert.equal(archivedList.status, 404);

          const crossAssignee = await request({
            method: 'POST',
            path: `/api/v1/homes/${homeA}/tasks`,
            headers: {
              Origin: TRUSTED_ORIGIN,
          ...
          assert.equal(crossAssignee.status, 400);
          assert.equal(
            (crossAssignee.json() as ApiErrorBody).error.code,
            'INVALID_REQUEST',
          );
          assertNoForbiddenLeak({
            context: 'cross-home assignee HTTP PG',
            text: crossAssignee.text,
            forbidden: [
              ...COMMON_SECRET_SENTINELS,
              membershipB,
              homeB,
          ...
          const leftover = await database.pool.query<{ count: string }>(
            `SELECT count(*)::text AS count FROM task_instances
             WHERE home_id = $1 AND title = 'Bad assignee'`,
            [homeA],
          );
