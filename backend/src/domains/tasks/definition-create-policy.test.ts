...
void describe('decideTaskDefinitionCreate', () => {
  void it('names the task_definition.create action', () => {
    assert.equal(TASK_DEFINITION_ACTION.create, 'task_definition.create');
  });
  ...
  void it('does not grant task_definition.create to an inactive Membership', () => {
    assert.equal(
      (TASK_DEFINITION_CREATE_CAPABLE_ROLES as readonly string[]).includes(
        'INACTIVE',
      ),
      false,
    );
  ...
  void it('has no Admin-bypass special case', async () => {
    ...
    assert.match(source, /TASK_DEFINITION_CREATE_CAPABLE_ROLES/);
  });
});
