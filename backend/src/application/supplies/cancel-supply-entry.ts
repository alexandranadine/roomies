...
import { decideSupplyCancel } from '../../domains/supplies/cancel-policy.js';
import {
  SupplyNotOpenError,
...
export type CancelSupplyEntryInput = Readonly<{
  actor: ActiveHomeActor;
  homeId: string;
  supplyEntryId: string;
}>;

export type CancelSupplyEntryDependencies = Readonly<{
  runTransaction: <T>(
...
/**
 * Cancels one OPEN SupplyEntry for an authorized Home actor.
 *
 ...
 * supply.cancel → SupplyEntry FOR UPDATE → complete OPEN matrix →
 * active SupplyClaim FOR UPDATE → one Clock.now() → optional ENTRY_CANCELED
 * release → conditional CANCELED terminalization.
 */
export function createCancelSupplyEntry(
  deps: CancelSupplyEntryDependencies,
): (input: CancelSupplyEntryInput) => Promise<SupplyEntry> {
  return async (input) => {
    const early = decideSupplyCancel({
      actor: input.actor,
    ...
    return deps.runTransaction(async (tx) => {
      ...
      const authorization = decideSupplyCancel({
        actor,
      ...
      const terminalize: TerminalizeSupplyEntryAsCanceled = Object.freeze({
        supplyEntryId: entry.id,
        homeId: locked.home.id,
        canceledAt: occurredAt,
      });
      const updated = await deps.supplies.terminalizeSupplyEntryAsCanceled(
        tx,
        terminalize,
      );
      if (updated === null) {
...
export function createCancelSupplyEntryFromPool(
  pool: TransactionPool,
): ReturnType<typeof createCancelSupplyEntry> {
