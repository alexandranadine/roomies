...
void describe('manual Task create concurrency PostgreSQL', () => {
  void it(
    ...
    async () => {
      ...
      try {
        ...
        assert.equal(
          await taskCount(database.pool, homeId, 'Create wins archive'),
          1,
        );
      } finally {
  ...
  void it(
    'archive winning the race conceals create and inserts no Task',
    { skip: skipWithoutDatabase, timeout: 60_000 },
    async () => {
      const database = createDatabasePool(
        testConfig(resolveSafeDedicatedTestDatabaseUrl()),
      ...
      try {
        ...
        assert.equal(
          await taskCount(database.pool, homeId, 'Archive wins create'),
          0,
        );
      } finally {
        await cleanup(database.pool, {
  ...
  void it(
    ...
    async () => {
      ...
      try {
        ...
        assert.equal(
          await taskCount(database.pool, homeId, 'Create wins actor-end'),
          1,
        );
      } finally {
        await cleanup(database.pool, {
  ...
  void it(
    'actor-end winning the race conceals create and inserts no Task',
    { skip: skipWithoutDatabase, timeout: 60_000 },
    async () => {
      const database = createDatabasePool(
        testConfig(resolveSafeDedicatedTestDatabaseUrl()),
      ...
      try {
        ...
        assert.ok(created.reason instanceof ConcealedNotFoundError);
        assert.equal(
          await taskCount(database.pool, homeId, 'Actor-end wins create'),
          0,
        );
      } finally {
        await cleanup(database.pool, {
  ...
  void it(
    'create winning the assignee-end race assigns while B is still active',
    { skip: skipWithoutDatabase, timeout: 60_000 },
    async () => {
      const database = createDatabasePool(
        testConfig(resolveSafeDedicatedTestDatabaseUrl()),
      ...
      try {
        ...
        await insertHome(database.pool, homeId, 'Assignee-end race A');
        await insertMembership(database.pool, {
          id: adminMembershipId,
          homeId,
          userId: adminId,
        ...
        const createRun = create({
          actor: admin,
          homeId,
          title: 'Create wins assignee-end',
          assignedMembershipId: assigneeMembershipId,
        });
        await createLocked.promise;
        ...
        await waitUntil(() => isWaitingForLock(database.pool, pid));
        assert.equal(
          await membershipEndedAt(database.pool, assigneeMembershipId),
          null,
        );

        createMayFinish.resolve();
        const created = await createRun;
        assert.equal(created.assignedMembershipId, assigneeMembershipId);
        assert.equal(
          await membershipEndedAt(database.pool, assigneeMembershipId),
          null,
        );
  ...
  void it(
    'assignee-end winning the race rejects assignment and inserts no Task',
    { skip: skipWithoutDatabase, timeout: 60_000 },
    async () => {
      const database = createDatabasePool(
        testConfig(resolveSafeDedicatedTestDatabaseUrl()),
      ...
      try {
        await insertUser(database.pool, adminId);
        await insertUser(database.pool, assigneeId);
        await insertHome(database.pool, homeId, 'Assignee-end race B');
        await insertMembership(database.pool, {
          id: adminMembershipId,
          homeId,
          userId: adminId,
        ...
        const createRun = observe(
          create({
            actor: admin,
            homeId,
            title: 'Assignee-end wins create',
            assignedMembershipId: assigneeMembershipId,
          }),
        );
        const pid = await createPid.promise;
        ...
        assert.equal(
          await taskCount(database.pool, homeId, 'Assignee-end wins create'),
          0,
        );
      } finally {
        await cleanup(database.pool, {
  ...
  void it(
    'repeated create-vs-structure races stay deadlock-free and never write after losing',
    ...
    async () => {
      ...
      try {
        for (let iteration = 0; iteration < STRESS_ITERATIONS; iteration += 1) {
          ...
          const title = `Archive stress ${iteration}`;
          const settled = await Promise.allSettled([
            ...
            archiveCommand(database.pool)({ homeId, actor: admin }),
          ]);
          const created = settled[0];
          const count = await taskCount(database.pool, homeId, title);
        ...
        for (let iteration = 0; iteration < STRESS_ITERATIONS; iteration += 1) {
          ...
          const adminMembershipId = randomUUID();
          const actorMembershipId = randomUUID();
          userIds.push(adminId, actorId);
          homeIds.push(homeId);
          await insertUser(database.pool, adminId);
          await insertUser(database.pool, actorId);
          await insertHome(database.pool, homeId, `Actor stress ${iteration}`);
          await insertMembership(database.pool, {
            id: adminMembershipId,
            homeId,
            userId: adminId,
          ...
          const title = `Actor stress ${iteration}`;
          const settled = await Promise.allSettled([
            createTaskCommand(database.pool)({
              actor: roommate,
              homeId,
        ...
        for (let iteration = 0; iteration < STRESS_ITERATIONS; iteration += 1) {
          ...
          const adminMembershipId = randomUUID();
          const assigneeMembershipId = randomUUID();
          userIds.push(adminId, assigneeId);
          homeIds.push(homeId);
          await insertUser(database.pool, adminId);
          await insertUser(database.pool, assigneeId);
          await insertHome(
          ...
          const title = `Assignee stress ${iteration}`;
          const settled = await Promise.allSettled([
            createTaskCommand(database.pool)({
              actor: admin,
              homeId,
        ...
        for (let iteration = 0; iteration < STRESS_ITERATIONS; iteration += 1) {
          ...
          await insertHome(database.pool, homeId, `Cross lock ${iteration}`);
          await insertMembership(database.pool, {
            id: membershipA,
            homeId,
            userId: userA,
          ...
          const [first, second] = await Promise.all([
            createTaskCommand(database.pool)({
              actor: actorA,
              homeId,
              title: `Cross A ${iteration}`,
              assignedMembershipId: membershipB,
            }),
            createTaskCommand(database.pool)({
              actor: actorB,
              homeId,
              title: `Cross B ${iteration}`,
              assignedMembershipId: membershipA,
            }),
          ]);
          assert.equal(first.assignedMembershipId, membershipB);
          assert.equal(second.assignedMembershipId, membershipA);
        }
      } finally {
  ...
  void it(
    'old tenure cannot create after rejoin; current tenure can',
    { skip: skipWithoutDatabase, timeout: 60_000 },
    async () => {
      const database = createDatabasePool(
        testConfig(resolveSafeDedicatedTestDatabaseUrl()),
      ...
      try {
        ...
        await assert.rejects(
          () =>
            create({
              ...
              homeId,
              title: 'Old tenure create',
            }),
          ConcealedNotFoundError,
        );
        assert.equal(
          await taskCount(database.pool, homeId, 'Old tenure create'),
          0,
        );
        ...
        const created = await create({
          ...
          homeId,
          title: 'New tenure create',
        });
        assert.equal(created.title, 'New tenure create');
      } finally {
        await cleanup(database.pool, {
          homeIds: [homeId],
